...
- 22:34 Runtime 已升级至 `1657509`，目标解析预检 v1 上线；全量 **530 passed**。公网 Mac/纯云端解析通过，未知工作区和仓库冲突返回结构化拒绝，四次预检未增加任务、工具操作或原生会话。详见 [目标预检发布记录](target-resolution-rollout-2026-09-14.md)。Mobile 可选预检尚待接入。
...
- Runtime 的 `ToolOperationRepository` facade 已落地并接入 API、ToolTransport、ToolOperationWorker；Runtime 全量 **494 passed**（UTF-8 环境，保留一个既有 Starlette/httpx warning）。当前 `TaskStore` 仍保留 NativeSession 的原子事务，下一刀再抽 `NativeSessionRepository`。
- `NativeSessionRepository` 已落地并接入 NativeToolExecutor、ToolTransport 和 Orchestrator；Runtime 全量 **495 passed**。它只隔离调用边界，session、tool operation 和同轮 event 的原子提交仍由 `TaskStore` 保持。
...
- Runtime follow-up `54528e6` 已部署，新增持久化 native Agent 重复调用保护：相同成功调用第 4 次前停止，相同失败达到阈值停止，显式重试清理 guard；Runtime 全量 **478 passed**。恢复矩阵见 [Runtime recovery matrix](../../pi-studio-control-plane/docs/native-tool-recovery-matrix-2026-09-13.md)。
...
- 下一步补断线/取消/恢复的真实跨端矩阵和可复现评测；同时等待 Mac 恢复在线后重跑 `local.list` happy path。不启动新的框架迁移。发布证据见 [Runtime 发布记录](../../pi-studio-control-plane/docs/tool-projection-rollout-2026-09-13.md)。
