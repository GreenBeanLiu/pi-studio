export type RoutineStepKind = 'agent' | 'folder-input' | 'imagegen' | 'review' | 'notify' | 'export' | 'feishu-doc' | 'wechat-draft'
...
export function isRoutineStepComplete(step: RoutineStepLike): boolean {
  ...
  if (step.type === 'folder-input' || step.type === 'review' || step.type === 'export' || step.type === 'feishu-doc' || step.type === 'wechat-draft') return true
